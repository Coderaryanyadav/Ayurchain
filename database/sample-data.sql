-- ====================================================================
-- Complete Ready-To-Test Demo Seed Data for AYURCHAIN
-- File: database/sample-data.sql
-- ====================================================================

USE `ayurvedic_blockchain`;

-- Clean prior sample rows
DELETE FROM `medicine_history` WHERE `medicine_id` IN ('AYU001', 'AYU002', 'AYU003');
DELETE FROM `blockchain_records` WHERE `medicine_id` IN ('AYU001', 'AYU002', 'AYU003');
DELETE FROM `medicine_documents` WHERE `medicine_id` IN ('AYU001', 'AYU002', 'AYU003');
DELETE FROM `medicines` WHERE `medicine_id` IN ('AYU001', 'AYU002', 'AYU003');

-- 1. Demo Medicines
INSERT INTO `medicines` (`medicine_id`, `medicine_name`, `botanical_name`, `category`, `form`, `manufacturer`, `manufacturing_date`, `expiry_date`, `batch_number`, `source`, `ingredients`, `dosage_instructions`, `description`, `blockchain_status`, `qr_code_path`) VALUES
('AYU001', 'Ashwagandha Churna', 'Withania somnifera', 'Churna', 'Herbal Powder', 'Dabur India Ltd - Ayurvedic Division', '2025-01-15', '2027-01-15', 'BATCH-ASH-2025-01', 'Madhya Pradesh Organic Cultivation Zone', 'Pure Ashwagandha (Withania somnifera) root extract powder', '1-2 teaspoons twice daily with warm milk or honey.', 'Premium standardized adaptogenic stress-relief herbal powder.', 'RECORDED', ''),
('AYU002', 'Triphala Guti', 'Terminalia chebula, bellerica, Emblica officinalis', 'Vati & Gutika', 'Standardized Tablet', 'Baidyanath Ayurvedic Bhawan', '2025-02-10', '2028-02-10', 'BATCH-TRI-2025-02', 'Vindhya Herbal Reserve, UP', 'Amalaki, Haritaki, Bibhitaki in equal proportions', '2 tablets before sleep with lukewarm water.', 'Classical digestive stimulant and colon detox formulation.', 'RECORDED', ''),
('AYU003', 'Mahanarayan Taila', 'Multiple Ayurvedic Herbs & Sesame Oil', 'Taila', 'Medicated Herbal Oil', 'Kottakkal Arya Vaidya Sala', '2025-03-01', '2028-03-01', 'BATCH-MAH-2025-03', 'Kerala Herbal Estate', 'Sesame oil base, Bilva, Ashwagandha, Bala, Shatavari, Rasna', 'Apply warm oil over affected joints and massage gently.', 'Classical formulation for neuromuscular and joint mobility support.', 'PENDING', '');

-- 2. Demo Lab Certificates & Hashes
...
-- 3. Demo Mined Blockchain Records
...
-- 4. Demo Supply Chain Provenance History
